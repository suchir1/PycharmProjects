...
class SVOSENT(object):
    ...
    def get_svo(self,sentence):
        ...
        for e in names:
            linked=e.text
            if any(map(str.isupper, linked)) and any(ext in linked for ext in ['January', 'February', 'March', 'April', 'May', 
                  'June', 'July', 'August', 'September', 'October', 'November', 'December','Sunday','Monday',
                  'Tuesday','Wednesday','Thursday','Friday','Saturday','week', 'Yesterday','month','day','Today'])==False:
        ...
        try:
            event_date=list(set(sentence.replace('.','').split())&set(['Monday','Tuesday','Wednesday','Tursday',
                                'Friday','Saturday','Sunday','Today','today', 
                                'Tomorrow','tomorrow', 'Yesterday', 'yesterday']))[0]
    ...
    def svo_senti_from_article(self,article, subject=None):
        ...
        result = pd.merge(pd.DataFrame(val1),pd.DataFrame(val2), on='Sentence')[['Sentence','Names','Persons','Organizations','Facilities','Locations','Subjects', 'Predicates', 'Objects', 'compound', 'pos','neu','neg','Event_date']]
#        try: 
#            result['date']=date
#        except:
#            result['date']='-----'
